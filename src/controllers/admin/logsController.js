...
exports.getStats = async (req, res, next) => {
    try {
        ...
        const stats = await Promise.all([
            ...
            query('SELECT COUNT(*) as count FROM crawl_jobs WHERE status = $1', ['running']),
            query(`SELECT COUNT(*) as count FROM crawl_jobs 
             WHERE status = $1 AND created_at > NOW() - INTERVAL '24 hours'`, ['completed'])
        ]);
